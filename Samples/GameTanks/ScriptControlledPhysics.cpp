...
void CScriptControlledPhysics::OnPostStep(EventPhysPostStep *pPostStep)
{
	...
	if (m_rotating)
	{
		...
		float angle=acosf(CLAMP(rotation.w, -1.0f, 1.0f))*2.0f;
		...
		else
		{
			...
			if (fabsf(m_rotationSpeed*dt)>fabsf(angle))
				m_rotationSpeed=angle/dt;
			else if (fabsf(m_rotationSpeed)<0.001f)
				m_rotationSpeed=sgn(m_rotationSpeed)*0.001f;
			else if (fabsf(m_rotationSpeed)>=m_rotationMaxSpeed)
			{
		...
		if(fabsf(angle)>=0.001f)
			av.w=(rotation.v/sinf(original*0.5f)).normalized();
