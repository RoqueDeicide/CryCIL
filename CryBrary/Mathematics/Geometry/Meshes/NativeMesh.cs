...
namespace CryEngine.Mathematics.Geometry.Meshes
{
	...
	public class NativeMesh : Mesh
	{
		...
		/// </param>
		/// <param name="warnings">
		/// A list that will contain a list of problems that may cause problems but won't prevent CryEngine
		/// from recognizing this mesh.
		/// </param>
		...
		/// </remarks>
		/// <param name="cache">   
		/// Indicates whether all collections must be transferred to Mono memory before validation. If
		/// true, can increase performance but along with memory consumption.
		/// </param>
		/// <param name="errors">  
		/// A list that will contain a list of problems that will prevent calling <see cref="Export"/>
		/// method from being successful.
		...
		public bool Validate(bool cache = false, List<string> errors = null, List<string> warnings = null)
		{
			...
			// Currently all meshes use UInt16 for indices, and it is not possible to change it without
			// access to engine source. CryMono uses UInt32 for indices, so, if there are too many
			// vertices, all extras will be discarded along with faces during export. When that happens, a
			// lot of data movements will have to happen, so it's best to avoid breaching a limit before
			// exporting.
