...
namespace CryEngine.Console.Variables
{
	...
	{
		/// <summary>
		/// just to have one recognizable spot where the flags are located in the Register
		/// call
		/// </summary>
		None = 0x00000000,
		...
		/// <summary>
		/// otherwise the const char * to the name will be stored without copying the
		/// memory
		/// </summary>
		CopyName = 0x00008000,
		...
		/// <summary>
		/// Always accept variable value and call on change callback even if variable
		/// value didnt change
		/// </summary>
		...
		/// <summary>
		/// This variable is critical to check in every hash, since it's extremely
		/// vulnerable
		/// </summary>
		CheatAlwaysCheck = 0x01000000,

		/// <summary>
		/// This variable is set as VF_CHEAT but doesn't have to be checked/hashed since
		/// it's harmless to workaround
		/// </summary>
		CheatNoCheck = 0x02000000,

		// These flags should never be set during cvar creation, and probably never set
		// manually.
		InternalFlagsStart = 0x00000080,

		/// <summary>
		/// can be changed on client and when connecting the var not sent to the client
		/// (is set for all vars in Game/scripts/Network/cvars.txt)
		/// </summary>
