...
{
	/// <summary>
	/// Represents a wrapper around a null terminated string stored in native memory that uses UTF-16.
	/// </summary>
	public class NTWString : INativeMemoryWrapper
	{
		#region Fields
		...
		public NTWString(string text)
		{
			this.handle = NativeString.GetNativeWideString(text);
			this.managing = true;
			this.disposed = false;
		...
		/// </param>
		public NTWString(IntPtr pointer, Allocators allocator, bool managing = false)
		{
			this.handle = pointer;
		...
		{
			return NativeString.GetMonoWideString(this.handle);
		}
		#endregion
		#region Utilities
		~NTWString()
