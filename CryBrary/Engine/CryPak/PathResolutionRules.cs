...
namespace CryEngine
{
	...
	{
		/// <summary>
		/// If used, the source path will be treated as the destination path and no
		/// transformations will be done. Pass this flag when the path is to be the actual
		/// path on the disk/in the packs and doesn't need adjustment (or after it has
		/// come through adjustments already) if this is set, AdjustFileName will not map
		/// the input path into the master folder (Ex: Shaders will not be converted to
		/// Game\Shaders)
		/// </summary>
		RealPath = 1 << 16,
		...
		/// <summary>
		/// if this is set, AdjustFileName will try to find the file under any mod paths
		/// we know about
		/// </summary>
		CheckModPaths = 1 << 27,

		/// <summary>
		/// if this is set, AdjustFileName will always check the filesystem/disk and not
		/// check inside open paks
		/// </summary>
		NeverInPaks = 1 << 28,
	}
}