...
{
	private:
	//! A list of registered classes of entities that interact with CryCIL.
	static List<Text> monoEntityClassNames;
	public:
...
struct EntitySlotsInterop : IMonoInterop<true, true>
{
	...
	static IMaterial            *GetSlotMaterial(IEntity *entityHandle, int nIndex);
	static int                   GetSlotParent(IEntity *entityHandle, int nIndex);
	...
	static int                   LoadParticleEmitter(IEntity *entityHandle, int slot, IParticleEffect *pEffect, SpawnParams *parameters, bool bPrime, bool bSerialize);
