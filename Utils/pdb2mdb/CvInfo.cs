...
namespace Microsoft.Cci.Pdb
{
	...
	// CodeView Symbol and Type OMF type information is broken up into two ranges. Type indices less than
	// 0x1000 describe type information that is frequently used. Type indices above 0x1000 are used to
	// describe more complex features such as functions, arrays and structures.

	// Primitive types have predefined meaning that is encoded in the values of the various bit fields in
	// the value.
	// 
	...
	// 
	// Where mode is the pointer mode type is a type indicator sub is a subtype enumeration r is a reserved
	// field
	// 
	...
	// selected values for type_index - for a more complete definition, see Microsoft Symbol and Type OMF
	// document
	...
	// the use of the type record. The second range is for the type records that are directly referenced in
	// symbols. The first range is for type records that are not referenced by symbols but instead are
	// referenced by other type records. All type records must have a starting leaf index in these first
	// two ranges. The third range of leaf indices are used to build up complex lists such as the field
	// list of a class type record. No type record can begin with one of the leaf indices. The fourth
	// ranges of type indices are used to represent numeric data in a symbol or type record. These leaf
	// indices are greater than 0x8000. At the point that type or symbol processor is expecting a numeric
	// field, the next two bytes in the type record are examined. If the value is less than 0x8000, then
	// the two bytes contain the numeric value. If the value is greater than 0x8000, then the data follows
	// the leaf index in a format specified by the leaf index. The final range of leaf indices are used to
	// force alignment of subfields within a complex type record..
	...
	// Type enum for pointer records Pointers can be one of the following types
	...
	// Mode enum for pointers Pointers can have one of the following modes
	...
	internal struct TYPTYPE
	{
		...
		// char *NextType (char * pType) { return (pType + ((TYPTYPE *)pType)->len + sizeof(ushort)); }
	};          // general types record

	// memory representation of pointer to member. These representations are indexed by the enumeration
	// above in the LF_POINTER record

	// representation of a 32 bit pointer to data for a class with or without virtual functions and no
	// virtual bases
	...
	// representation of a 32 bit pointer to member function for a class with no virtual bases and a single
	// address point
	...
	// representation of a 32 bit pointer to member function for a class with no virtual bases and multiple
	// address points
	...
	// Notes on alignment Alignment of the fields in most of the type records is done on the basis of the
	// TYPTYPE record base. That is why in most of the lf* records that the type is located on what appears
	// to be a offset mod 4 == 2 boundary. The exception to this rule are those records that are in a list
	// (lfFieldList,
	// lfMethodList) , which are aligned to their own bases since they don't have the length field
	...
	// list leaf This list should no longer be used because the utilities cannot verify the contents of the
	// list without knowing what type of list it is. New specific leaf indices should be used instead.
	...
	internal struct LeafDimVar
	{
		...
		// variable upper bound or variable lower/upper bound. The count of type indices is rank or rank*2
		// depending on whether it is LFDIMVARU or LF_DIMVARLU. The referenced types must be LF_REFSYM or
		// T_VOID
	};
	...
	// the following are numeric leaves. They are used to indicate the size of the following variable
	// length data. When the numeric data is a single byte less than 0x8000, then the data is output
	// directly. If the data is more the 0x8000 or is a negative value, then the data is preceeded by the
	// proper index.
	...
	// to emit a long complex list (LF_FIELD) in smaller pieces.
	...
	// type record for nested (scoped) type definition, with attributes new records for vC v5.0, no need to
	// have 16-bit ti versions.
	...
	internal struct SYMTYPE
	{
		...
		// byte data[CV_ZEROLEN]; SYMTYPE *NextSym (SYMTYPE * pSym) { return (SYMTYPE *) ((char *)pSym +
		// pSym->reclen + sizeof(ushort)); }
	};
	...
	{
		// internal ushort reclen; // Record length [SYMTYPE] internal ushort rectyp; // S_MANREGISTER |
		// S_ATTR_REGISTER
		internal uint typind;     // (type index) Type index or Metadata token
	...
	{
		// internal ushort reclen; // Record length [SYMTYPE] internal ushort rectyp; // S_MANMANYREG2 |
		// S_ATTR_MANYREG
		internal uint typind;     // (type index) Type index or metadata token
		internal uint offCod;     // first code address where var is live
		internal ushort segCod;
	...
	{
		// internal ushort reclen; // Record length [SYMTYPE] internal ushort rectyp; // S_CONSTANT or
		// S_MANCONSTANT
	...
	{
		// internal ushort reclen; // Record length [SYMTYPE] internal ushort rectyp; // S_MANFRAMEREL |
		// S_ATTR_FRAMEREL
	...
	{
		// internal ushort reclen; // Record length [SYMTYPE] internal ushort rectyp; // S_LOCALSLOT or
		// S_PARAMSLOT
	...
	{
		// internal ushort reclen; // Record length [SYMTYPE] internal ushort rectyp; // S_GPROC32 or
		// S_LPROC32
	...
	{
		// internal ushort reclen; // Record length [SYMTYPE] internal ushort rectyp; // S_GMANPROCMIPS or
		// S_LMANPROCMIPS
		internal uint parent;     // pointer to the parent
		internal uint end;        // pointer to this blocks end
		internal uint next;       // pointer to next symbol
	...
	{
		// internal ushort reclen; // record length internal ushort rectyp; // S_VFTABLE32
	...
	{
		// internal ushort reclen; // record length internal ushort rectyp; // S_SLINK32
	...
	{
		// internal ushort reclen; // Record length [SYMTYPE] internal ushort rectyp; // S_GPROCMIPS or
		// S_LPROCMIPS
		internal uint parent;     // pointer to the parent
		internal uint end;        // pointer to this blocks end
		internal uint next;       // pointer to next symbol
	...
	{
		// internal ushort reclen; // Record length [SYMTYPE] internal ushort rectyp; // S_GPROCIA64 or
		// S_LPROCIA64
		internal uint parent;     // pointer to the parent
		internal uint end;        // pointer to this blocks end
		internal uint next;       // pointer to next symbol
	...
	{
		// internal ushort reclen; // Record length [SYMTYPE] internal ushort rectyp; // S_PROCREF_ST,
		// S_DATAREF_ST, or S_LPROCREF_ST
	...
	{
		// internal ushort reclen // Record length internal ushort rectyp; // S_SECTION
	...
	// Symbol for describing indirect calls when they are using a function pointer cast on some other type
	// or temporary. Typical content will be an LF_POINTER to an LF_PROCEDURE type record that should mimic
	// an actual variable with the function pointer type in question.
	// 
	// Since the compiler can sometimes tail-merge a function call through a function pointer, there may be
	// more than one S_CALLSITEINFO record at an address. This is similar to what you could do in your own
	// code by:
	// 
