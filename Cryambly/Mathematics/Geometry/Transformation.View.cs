namespace CryCil.Geometry
{
	public static partial class Transformation
	{
		...
		public static class View
		{
			...
			/// <overloads>
			/// <summary> Creates a look-at transformation matrix that transforms vector from world space
			/// to camera space. </summary><remarks> When global Up vector is not specified, then default
			...
			public static Matrix44 Create(Vector3 eye, Vector3 at)
			{
				...
				Vector3 zaxis = (at - eye).Normalized;
				// X-axis points to the left from the camera (this is why we need an Up vector).
				Vector3 xaxis = new Vector3(-zaxis.Y, zaxis.X, 0);	// This is a cross-product of the default Up vector and a Z-axis that we calculated above.
				// Y-axis points upwards from the camera.
				Vector3 yaxis = xaxis % zaxis;
