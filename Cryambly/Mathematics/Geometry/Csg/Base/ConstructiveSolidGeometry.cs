...
namespace CryCil.Geometry.Csg.Base
{
	#region High Level Implementation
	/// <summary>
	...
	public static class ConstructiveSolidGeometry
	{
		...
		public static Solid Subtract(Solid a, Solid b)
		{
			...
			aNode.ClipTo(bNode);
			bNode.ClipTo(aNode);
			// Union of inverted A with B.
			bNode.Invert();
			bNode.ClipTo(aNode);
			bNode.Invert();
			aNode.Build(bNode.AllPolygons);

			// Invert everything since what we've got above is what we need but it's inside out.
			aNode.Invert();
			return new Solid(aNode.AllPolygons);
		}
		/// <summary>
		...
		public class Solid
		{
									   ...
									   Vector3? lengthsSet = null)
			{
				...
				Vector3 halfs = lengths.Scaled(1.0f / 2);
									   ...
									   float stacks = 8.0f)
			{
				...
				Func<float, float, Vertex> createVertex = delegate(float theta, float phi)
				{
					...
					Vector3 direction = new Vector3(vx: (float)(Math.Cos(theta) * Math.Sin(phi)),
										 ...
										 float slices = 16.0f)
			{
				...
				Vector3 axisX = new Vector3(Convert.ToInt32(isY),
											Convert.ToInt32(!isY),
											0).Cross(axisZ).Normalized;
		...
		public class Polygon
		{
			...
			/// <exception cref="ArgumentException">Not enough vertices.</exception>
		...
		public class Plane
		{
			...
			/// </summary>
			/// <remarks>
			/// <para>Normal is initialized to <see cref="Vector3.Forward"/>.</para>
			/// <para>W is initialized to 0.</para>
			/// </remarks>
			public Plane()
			{
		...
		/// <summary>
		/// Represents a vector with 3 coordinates.
		/// </summary>
		public struct Vector3
		{
			...
			/// <summary>
			/// Creates a vector by scaling this one.
			/// </summary>
			/// <param name="scaleFactor">Scale factor.</param>
			/// <returns>New vector with scaled coordinates.</returns>
		...
		public class Node
		{
			...
			public List<Polygon> AllPolygons
			{
				get
				{
					...
					List<Polygon> allPolygons = new List<Polygon>(this.Polygons.Count + frontPolycount + backPolycount);
