...
namespace CryCil.Hashing
{
	...
	public unsafe struct Crc32
	{
		...
		#region Interface
		/// <summary>
		/// Bakes the hash code of the string into current hash code instance.
		/// </summary>
		...
		public void Add(string text)
		{
			...
			fixed (char* chars = text)
			{
				...
				this.Add(chars, byteCount);
			}
		}
		/// <summary>
		/// Bakes the hash code of the data chunk into current hash code instance.
		/// </summary>
		...
		public void Add(void* data, int byteCount)
		{
			if (!this.initialized)
			{
				this.hash = uint.MaxValue;
		...
		public void AddLowercase(string text)
		{
			...
			{
				int byteCount = text.Length * 2;
				this.AddLowercase(chars, byteCount);
			}
		}
		/// <summary>
		/// Bakes the hash code of the data chunk (where each ASCII character is converted into lower case)
		/// into current hash code instance.
		/// </summary>
		/// <param name="data">     A pointer to the beginning of data chunk.</param>
		/// <param name="byteCount">Number of bytes in the data chunk.</param>
		public void AddLowercase(void* data, int byteCount)
		{
			if (!this.initialized)
			{
				this.hash = uint.MaxValue;
			...
			{
				byte b = *bytes++;
				b = b <= zUpper && b >= aUpper ? (byte)(b + UpperToLower) : b;
				this.hash = (this.hash >> 8) ^ table[this.hash & 0xFF] ^ b;
			}
		}
		/// <summary>
		/// Computes hash code of the text data using CRC32 hash algorithm.
		/// </summary>
		/// <param name="text">Text which hash code needs to be calculated.</param>
		/// <returns>Hash code of the given string.</returns>
		public static uint Compute(string text)
		{
			Crc32 crc;
			crc.hash = uint.MaxValue;
		...
		/// <summary>
		/// Computes hash code of the data chunk using CRC32 hash algorithm.
		/// </summary>
		/// <param name="data">     A pointer to the beginning of data chunk.</param>
		/// <param name="byteCount">Number of bytes in the data chunk.</param>
		/// <returns>Hash code of the given data.</returns>
		public static uint Compute(void* data, int byteCount)
		{
			Crc32 crc;
			crc.hash = uint.MaxValue;
			crc.initialized = true;

			crc.Add(data, byteCount);
			return crc.hash;
		}
		/// <summary>
		/// Computes hash code of the all-lower-case version of text data using CRC32 hash algorithm.
		/// </summary>
		/// <param name="text">Text which hash code needs to be calculated.</param>
		/// <returns>Hash code of the given string.</returns>
		public static uint ComputeLowercase(string text)
		{
			Crc32 crc;
			crc.hash = uint.MaxValue;
			crc.initialized = true;

			crc.AddLowercase(text);
			return crc.hash;
		}
		/// <summary>
		...
		/// <param name="byteCount">Number of bytes in the data chunk.</param>
		/// <returns>Hash code of the given data.</returns>
		public static uint ComputeLowercase(void* data, int byteCount)
		{
			Crc32 crc;
			crc.hash = uint.MaxValue;
			crc.initialized = true;

			crc.AddLowercase(data, byteCount);
			return crc.hash;
		}
		#endregion
	}
